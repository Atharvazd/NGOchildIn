...
var DonationModel = require('../../models/api/DonationModel');
...
function getDonationsByChildId(req, res) {
    DonationModel.getDonationsByChildId(req.params.id, function(err, model) {
        if (!err)
            res.json(model);
        else {
            res.send(500, err)
...
function insertDonation(req, res) {
    DonationModel.insertDonation(req.body.DonorId, req.body.ChildId, req.body.Amount, req.body.DonatedOn, req.body.description, function(err, model) {
        if (!err) {
            ...
            res.send({
                SUCCESS: true,
                DonationId: model.insertId,
                message: 'donation insterted successfully'
            });
        } else {
            res.send(500, err);
        }
